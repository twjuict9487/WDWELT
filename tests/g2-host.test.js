...
function fixture() {
  ...
  writeFileSync(join(root, 'build-metadata.json'), JSON.stringify({ releaseLabel: 'G2 Pilot', version: '0.2.0', build: 'test-42', buildTimestamp: '2026-09-02T00:00:00.000Z' }));
  return root;
}

async function withServer(root, operation) {
  const server = createServer(createRequestHandler({ root, startedAt: Date.now() - 2_500 }));
...
describe('G2 production host', () => {
  it('serves index, hashed assets, SPA routes and exact health metadata', async () => {
    ...
    await withServer(root, async (url) => {
      ...
      expect(await health.json()).toMatchObject({ status: 'ok', version: '0.2.0', build: 'test-42' });
  ...
  it('rotates only matching WDWELT logs inside its configured directory', () => {
    ...
    const log = createLogger({ logPath: logs, maxBytes: 50, retentionDays: 14, release: { version: '0.2.0', build: '42' } });
    log('info', 'start', 'test');
